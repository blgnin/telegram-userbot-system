...
# Определяем режим работы (локальный или продакшен)
...
if not RENDER_MODE:
    ...
    # Продакшен режим - используем системные переменные
    print("☁️ Режим: Render.com продакшен")
...
# Отладочная информация (скрываем номера в продакшене)
